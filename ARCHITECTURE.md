...
scope = spec   (flat under crates/)   NDN community specs implemented faithfully
  ...
  ndn-repo                      Persistent named-data repository (third-party durable custody, network-layer/app-agnostic). RepoCmd wire codec byte-compatible with ndnd (SyncJoin/SyncLeave/BlobFetch); transport-agnostic Repo engine over a *pluggable* DataStore — MemoryStore (ndn-sync) or the on-disk FjallStore here. SVS-group ingestion (`ingest_group` drives `SvSync::ingest_publication`, storing raw wires the shared store then re-serves); blob push-insertion; serve-by-name. Distributed replication/failover (cf. a-thieme/repo) is a layer above.
