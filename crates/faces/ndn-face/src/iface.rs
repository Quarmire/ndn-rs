//! Cross-platform network interface enumeration with glob-based
//! whitelist/blacklist filtering for face-system auto-configuration.
...
/// Blacklist is checked first (any match → denied), then whitelist (empty =
/// allow all).
...
/// Glob matcher supporting `*` (any sequence) and `?` (one char).
...
fn list_interfaces_windows() -> Vec<InterfaceInfo> {
    ...
    unsafe {
        ...
        while !adapter.is_null() {
            ...
            // Windows has no per-adapter multicast flag; treat all non-loopback
            // UP adapters as multicast-capable.
...
mod tests {
    ...
    fn interface_allowed_basic() {
        ...
        assert!(!interface_allowed("virbr0", &wl, &bl));
    }
