...
/// Delegates to an inner strategy and post-processes its forwarding actions
/// through a chain of filters. Enables cross-layer filtering without
/// modifying base strategies (e.g. `BestRouteStrategy` + `RssiFilter` →
/// "best route on faces with acceptable signal strength").
