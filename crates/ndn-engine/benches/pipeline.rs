...
use ndn_runtime::Spawn as RuntimeSpawn;
...
fn bench_pit_check(c: &mut Criterion) {
    ...
    group.bench_function("new_entry", |b| {
        ...
        b.iter(|| {
            ...
            c.name = Some(interest.name.clone());
            c.packet = DecodedPacket::Interest(Box::new(interest));
            let action = rt.block_on(stage.process(c));
            debug_assert!(matches!(action, Action::Continue(_)));
        });
    });
...
fn bench_fib_lpm(c: &mut Criterion) {
    ...
    for &n_routes in &[10, 100, 1000] {
        ...
        for i in 0..n_routes {
            ...
            fib.add_nexthop(&prefix, FaceId(((i % 10) + 2) as u64), 10);
        }
...
fn bench_pit_match(c: &mut Criterion) {
    ...
    group.bench_function("hit", |b| {
        ...
        b.iter(|| {
            ...
            let pit = Arc::new(Pit::new());
            let interest = ndn_packet::Interest::decode(interest_bytes.clone()).unwrap();
            let token = PitToken::from_interest_full(&interest.name, interest.forwarding_hint());
            let mut entry = ndn_store::PitEntry::new(interest.name.clone(), 0, 4000);
            entry.add_in_record(
                1,
                interest.nonce().unwrap_or(0),
                u64::MAX,
                None,
                interest.selectors().clone(),
            );
        ...
        group.bench_with_input(BenchmarkId::new("no_route", n), &wire, |b, wire| {
            ...
            b.iter(|| {
                rt.block_on(async {
                    ...
                    // PIT check (new entry)
                    let _c = match pit_check.process(c).await {
                        Action::Continue(c) => c,
...
// ─── Spawn overhead (wasm-phase-1 regression probe) ─────────────────────────
//
// Compares three spawn paths on a 4-worker multi-thread runtime, using batches
// of BATCH_SIZE=64 tasks (matching PacketDispatcher::BATCH_SIZE):
//
//   spawn_concrete      — direct tokio::spawn(concrete_future)
//                         "before" baseline: single allocation (task+future inline)
//   spawn_boxed         — tokio::spawn(Box::pin(concrete_future))
//                         "after" state:    two allocations (Box + task header)
//   runtime_trait_boxed — TokioRuntime::spawn(Box::pin(...))
//                         "after + vtable":  same as spawn_boxed plus one indirect call
//
// Per-spawn cost = measured batch time ÷ BATCH_SIZE.
// Overhead ratio = (spawn_boxed_mean - spawn_concrete_mean) / spawn_concrete_mean.

fn bench_spawn_overhead(c: &mut Criterion) {
    const BATCH: usize = 64; // matches PacketDispatcher::BATCH_SIZE
    ...
    // "before": direct tokio::spawn with concrete future — one allocation
    group.bench_function("spawn_concrete", |b| {
        b.iter(|| {
            rt.block_on(async {
    ...
    // "after": Box::pin + tokio::spawn — two allocations
    group.bench_function("spawn_boxed", |b| {
        b.iter(|| {
            rt.block_on(async {
                let mut handles = Vec::with_capacity(BATCH);
                for i in 0..BATCH {
    ...
    // "after + trait dispatch": TokioRuntime::spawn(Box::pin(...)) — two allocations + vtable
    // No JoinHandle from the trait; use a semaphore to wait for all tasks.
    {
        let runtime = Arc::new(TokioRuntime);
        group.bench_function("runtime_trait_boxed", |b| {
            b.iter(|| {
                rt.block_on(async {
                    ...
                    let _ = sem.acquire_many(BATCH as u32).await.unwrap();
                })
            })
        });
    }
