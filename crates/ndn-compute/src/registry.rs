...
/// Compute handler invoked for Interests whose name longest-prefix-matches
/// a registered name. The returned Data is injected back into the pipeline
/// and cached in the engine CS like any other Data.
...
pub enum ComputeError {
    ...
    /// The invocation name could not be decoded into the function's arguments.
    BadArguments(String),
}
...
impl std::fmt::Display for ComputeError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ComputeError::BadArguments(e) => write!(f, "bad compute arguments: {e}"),
        }
    }
}
