...
//!
//! ```text
//! Hello Interest: /ndn/local/nd/hello/<nonce-u32>  (no AppParams)
//! Hello Data:     /ndn/local/nd/hello/<nonce-u32>  Content = HelloPayload TLV
//! ```
//!
//! Sender MAC extracted from `meta.source`, not from the packet payload.

use std::ops::{Deref, DerefMut};
use std::time::{Duration, Instant};

use bytes::Bytes;
use ndn_faces::l2::{NamedEtherFace, RadioFaceMetadata};
use ndn_transport::{FaceId, MacAddr};

use crate::hello::medium::{HELLO_PREFIX_DEPTH, HelloCore, HelloState, LinkMedium};
use crate::wire::{parse_raw_interest, write_name_tlv, write_nni};
use crate::{
    ...
    fn handle_hello_interest(
        ...
        if is_new {
            core.strategy
                .lock()
                .unwrap()
                .trigger(crate::TriggerEvent::PassiveDetection);
        }
...
mod tests {
    ...
    fn protocol_id_and_prefix() {
        ...
        assert_eq!(
            nd.core.claimed[0],
            Name::from_str(crate::hello::medium::HELLO_PREFIX_STR).unwrap()
        );
    }
