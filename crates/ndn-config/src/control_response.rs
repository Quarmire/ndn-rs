...
pub mod status {
    ...
    /// Target resource is locked to non-authorised callers (HTTP 423);
    /// distinct from `UNAUTHORIZED` ("never permitted" vs "not
    /// authenticated").
    pub const LOCKED: u64 = 423;
    pub const SERVER_ERROR: u64 = 500;
    /// Verb unavailable in this build (HTTP 501), e.g. native-only
    /// modules dispatched against a wasm-built `ndn-mgmt`.
    pub const NOT_IMPLEMENTED: u64 = 501;
    /// Transport/link service doesn't implement the requested option
    /// (HTTP 503), e.g. `LpReliability` on a Shm face.
    pub const SERVICE_UNAVAILABLE: u64 = 503;
}
...
pub struct ControlResponse {
    ...
    /// Typically echoed ControlParameters on success.
