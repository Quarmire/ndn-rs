//! Consumer-side window-based congestion control. Reacts to Data arrivals,
//! NDNLPv2 CongestionMark, and Interest timeouts.

/// Consumer-side congestion control algorithm. Caller drives `on_data`,
/// `on_congestion_mark`, `on_timeout` and reads `window()`.
///
/// `Aimd`: linear increase, multiplicative decrease (matches `ndncatchunks`).
/// `Cubic`: RFC 8312, faster ramp-up on high-BDP links.
/// `Fixed`: constant window, no adaptation.
#[derive(Debug, Clone)]
pub enum CongestionController {
    Aimd {
        ...
        /// Slow-start threshold; below it window grows +1 per ack.
        ssthresh: f64,
    },
    /// CUBIC (RFC 8312).
    Cubic {
...
impl CongestionController {
    ...
    /// Set the initial and current window. For Cubic, also raises `w_max`
    /// so the cubic recovery target reflects the caller's intent.
    ...
    /// Default 0.5 for AIMD, 0.7 for CUBIC.
    ...
    /// Default is `f64::MAX` (unbounded slow start). Set to the initial
    /// window to skip exponential ramp.
    ...
    /// Use `window().floor() as usize` for the in-flight limit.
    ...
    /// Reduces window; does not trigger retransmission.
...
mod tests {
    ...
    fn cubic_does_not_collapse_at_large_initial_window() {
        ...
        assert!(
            cc.window() >= 64.0,
            "first on_data shrank window to {}",
            cc.window()
        );
