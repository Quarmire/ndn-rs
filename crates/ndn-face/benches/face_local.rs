    ...
    #[cfg(unix)]
    {
        ...
        for &size in &[64_usize, 1_024, 8_192] {
            ...
            let (fa, fb) = rt.block_on(async {
                ...
                let fa = ndn_face::local::unix_face_from_stream(FaceId(20), s1, "pair-a");
                let fb = ndn_face::local::unix_face_from_stream(FaceId(21), s2, "pair-b");
                (fa, fb)
            });
            ...
            group.bench_with_input(BenchmarkId::from_parameter(size), &pkt, |b, pkt| {
                b.iter(|| {
                    rt.block_on(async {
                        ...
                        tokio::join!(
                            async {
                                fa.send_bytes(pkt.clone()).await.unwrap();
                                fa.recv_bytes().await.unwrap();
                            },
                            async {
                                fb.recv_bytes().await.unwrap();
                                fb.send_bytes(pkt.clone()).await.unwrap();
                            },
                        );
                    });
                });
...
fn bench_unix_throughput(c: &mut Criterion) {
    #[cfg(unix)]
    {
        ...
        for &size in &[64_usize, 1_024, 8_192] {
            ...
            let (fa, fb) = rt.block_on(async {
                let (s1, s2) = UnixStream::pair().unwrap();
                let fa = ndn_face::local::unix_face_from_stream(FaceId(22), s1, "pair-c");
                let fb = ndn_face::local::unix_face_from_stream(FaceId(23), s2, "pair-d");
                (fa, fb)
            });
