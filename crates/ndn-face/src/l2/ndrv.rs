//! macOS raw Ethernet via `PF_NDRV` (Network Driver Raw), the Darwin protocol
//! family for unregistered EtherTypes such as NDN's 0x8624.
//!
//! PF_NDRV delivers full Ethernet frames (no SOCK_DGRAM header stripping), so
//! the source MAC is read from bytes [6..12] and TX must prepend
//! `[dst][src][0x86 0x24]` — the `send_to`/`send_to_mcast` helpers do that.
//! The NDN multicast group `01:00:5e:00:17:aa` is joined via
//! `NDRV_ADDMULTICAST` after bind. Requires root.
...
/// Simplified `sockaddr_dl` (just enough for the multicast-join MAC).
#[repr(C)]
struct SockaddrDl {
    sdl_len: u8,
    sdl_family: u8,
...
/// Async PF_NDRV socket bound to an Ethernet interface. Receives/sends full
/// raw frames for EtherType 0x8624.
...
impl NdrvSocket {
    /// Opens a PF_NDRV socket, registers EtherType 0x8624, and joins the NDN
    /// multicast group. Requires root.
    ...
    /// Returns `(payload, source_mac)` with the 14-byte Ethernet header
    /// stripped; source MAC is read from frame bytes [6..12].
    ...
    /// Prepends `[dst_mac][local_mac][0x86 0x24]` and sends the frame.
