...
/// [`Service::typ`] value marking a trusted-approver entry: an identity
/// permitted to approve NDNCERT device-approval enrollments for this subject
/// (the published form of `trustedApprovers`). The endpoint is the approver's
/// NDN identity name.
pub const TRUSTED_APPROVER_SERVICE_TYPE: &str = "NdnTrustedApprover";
...
impl DidDocument {
    ...
    /// Identities listed as trusted approvers — service entries of type
    /// [`TRUSTED_APPROVER_SERVICE_TYPE`]. These are the devices permitted to
    /// approve NDNCERT device-approval enrollments for this subject.
    pub fn trusted_approvers(&self) -> Vec<String> {
        self.service
            .iter()
            .filter(|s| s.typ == TRUSTED_APPROVER_SERVICE_TYPE)
            .filter_map(|s| match &s.service_endpoint {
                ServiceEndpoint::Uri(uri) => Some(uri.clone()),
                _ => None,
            })
            .collect()
    }

    /// Add a trusted-approver service entry (builder).
    pub fn with_trusted_approver(mut self, approver: impl Into<String>) -> Self {
        let id = format!("{}#approver-{}", self.id, self.service.len());
        self.service.push(Service {
            id,
            typ: TRUSTED_APPROVER_SERVICE_TYPE.to_string(),
            service_endpoint: ServiceEndpoint::Uri(approver.into()),
        });
        self
    }
