//! Conversion between NDN [`Certificate`]s and `DidDocument`s.
//!
//! An NDN certificate is structurally equivalent to a DID Document:
//! the namespace IS the identifier, and the certificate's public key
//! is the verification method. This module makes that equivalence explicit.

use std::sync::Arc;

use bytes::Bytes;
use ndn_packet::{Name, tlv_type};
use ndn_tlv::{TlvReader, TlvWriter};

use crate::Certificate;
use crate::did::{
    document::{DidDocument, VerificationMethod, VerificationRef},
    encoding::name_to_did,
};

const GENERIC_NAME_COMPONENT: u64 = 8;
const KEY_COMPONENT: &[u8] = b"KEY";

/// `DescriptionKey` under which a certificate's `AdditionalDescription` carries
/// the subject's published trusted-approver list (a JSON array of approver
/// identity strings). Signed by the cert's issuer, so the list is published and
/// tamper-evident — surfaced as [`TRUSTED_APPROVER_SERVICE_TYPE`] services on
/// the resolved DID Document.
///
/// [`TRUSTED_APPROVER_SERVICE_TYPE`]: crate::did::TRUSTED_APPROVER_SERVICE_TYPE
pub const TRUSTED_APPROVERS_DESCRIPTION_KEY: &str = "ndn.trusted-approvers";

/// Convert an NDN certificate to a W3C `DidDocument`.
///
/// The certificate's `name` is expected to be a KEY name like
/// `/com/acme/alice/KEY/v=123/self`. The identity DID is derived from
/// the prefix before `/KEY/`.
///
/// # Key agreement key
///
/// Pass `x25519_key` to include a `keyAgreement` entry. This is an X25519
/// public key separate from the Ed25519 signing key — critical for NDA's
/// encrypted content tier. The X25519 key is typically derived from the
/// Ed25519 seed (e.g., via RFC 7748 conversion) or generated independently.
pub fn cert_to_did_document(cert: &Certificate, x25519_key: Option<&[u8]>) -> DidDocument {
    let identity_name = strip_key_suffix(cert.name.as_ref());
    let did = name_to_did(&identity_name);
    let key_id = format!("{did}#key-0");

    let vm = VerificationMethod::ed25519_jwk(&key_id, &did, &cert.public_key);

    let mut doc = DidDocument {
        context: vec![
            "https://www.w3.org/ns/did/v1".to_string(),
            "https://w3id.org/security/suites/jws-2020/v1".to_string(),
        ],
        id: did.clone(),
        controller: None,
        verification_methods: vec![vm],
        authentication: vec![VerificationRef::Reference(key_id.clone())],
        assertion_method: vec![VerificationRef::Reference(key_id.clone())],
        key_agreement: vec![],
        capability_invocation: vec![VerificationRef::Reference(key_id.clone())],
        capability_delegation: vec![VerificationRef::Reference(key_id)],
        service: vec![],
        also_known_as: vec![],
    };

    if let Some(x25519_bytes) = x25519_key {
        let ka_id = format!("{did}#key-agreement-0");
        let ka_vm = VerificationMethod::x25519_jwk(&ka_id, &did, x25519_bytes);
        doc.verification_methods.push(ka_vm);
        doc.key_agreement.push(VerificationRef::Reference(ka_id));
    }

    if let Some(issuer) = &cert.issuer {
        let issuer_identity = strip_key_suffix(issuer.as_ref());
        let issuer_did = name_to_did(&issuer_identity);
        if issuer_did != did {
            doc.also_known_as.push(issuer_did);
        }
    }

    // Surface any published trusted-approver list (from the cert's signed
    // AdditionalDescription) as service entries.
    for approver in trusted_approvers_from_cert(cert) {
        doc = doc.with_trusted_approver(approver);
    }

    doc
}

/// Read the published trusted-approver list from a cert's signed
/// `AdditionalDescription`. Empty when absent or malformed.
pub fn trusted_approvers_from_cert(cert: &Certificate) -> Vec<String> {
    let Some(signed_region) = cert.signed_region.as_ref() else {
        return Vec::new();
    };
    let Some(value) = read_description_value(signed_region, TRUSTED_APPROVERS_DESCRIPTION_KEY)
    else {
        return Vec::new();
    };
    serde_json::from_slice(&value).unwrap_or_default()
}

/// Encode the **value** of an `AdditionalDescription` (0x0102) TLV carrying the
/// trusted-approver list — a single `DescriptionEntry`
/// (`TRUSTED_APPROVERS_DESCRIPTION_KEY` → JSON array). Pass to
/// [`SecurityManager::certify_with_additional_description`](crate::SecurityManager::certify_with_additional_description)
/// when issuing the subject's cert.
pub fn encode_trusted_approvers_description(approvers: &[String]) -> Vec<u8> {
    let json = serde_json::to_vec(approvers).unwrap_or_else(|_| b"[]".to_vec());
    let mut w = TlvWriter::new();
    w.write_nested(tlv_type::DESCRIPTION_ENTRY, |w| {
        w.write_tlv(
            tlv_type::DESCRIPTION_KEY,
            TRUSTED_APPROVERS_DESCRIPTION_KEY.as_bytes(),
        );
        w.write_tlv(tlv_type::DESCRIPTION_VALUE, &json);
    });
    w.finish().to_vec()
}

/// Walk `signed_region` → `SignatureInfo` → `AdditionalDescription` and return
/// the `DescriptionValue` of the entry whose `DescriptionKey` equals `key`.
fn read_description_value(signed_region: &[u8], key: &str) -> Option<Bytes> {
    let mut reader = TlvReader::new(Bytes::copy_from_slice(signed_region));
    while !reader.is_empty() {
        let (typ, val) = reader.read_tlv().ok()?;
        if typ != tlv_type::SIGNATURE_INFO {
            continue;
        }
        let mut si = TlvReader::new(val);
        while !si.is_empty() {
            let (stype, sval) = si.read_tlv().ok()?;
            if stype != tlv_type::ADDITIONAL_DESCRIPTION {
                continue;
            }
            let mut ad = TlvReader::new(sval);
            while !ad.is_empty() {
                let (etype, eval) = ad.read_tlv().ok()?;
                if etype != tlv_type::DESCRIPTION_ENTRY {
                    continue;
                }
                let mut e = TlvReader::new(eval);
                let mut found = false;
                let mut value = None;
                while !e.is_empty() {
                    let (t, v) = e.read_tlv().ok()?;
                    match t {
                        t if t == tlv_type::DESCRIPTION_KEY => found = v.as_ref() == key.as_bytes(),
                        t if t == tlv_type::DESCRIPTION_VALUE => value = Some(v),
                        _ => {}
                    }
                }
                if found && value.is_some() {
                    return value;
                }
            }
        }
    }
    None
}

/// Attempt to reconstruct a trust anchor [`Certificate`] from a `DidDocument`.
///
/// Returns `None` if the document does not contain a recognised Ed25519 key.
pub fn did_document_to_trust_anchor(doc: &DidDocument, name: Arc<Name>) -> Option<Certificate> {
    let key_bytes = doc.ed25519_public_key()?;
    Some(Certificate {
        name,
        public_key: bytes::Bytes::copy_from_slice(&key_bytes),
        valid_from: 0,
        valid_until: u64::MAX,
        issuer: None,
        signed_region: None,
        sig_value: None,
        sig_type: ndn_packet::SignatureType::SignatureEd25519,
    })
}

pub(crate) fn strip_key_suffix(name: &Name) -> Name {
    let comps = name.components();
    let key_pos = comps
        .iter()
        .rposition(|c| c.typ == GENERIC_NAME_COMPONENT && c.value.as_ref() == KEY_COMPONENT);
    match key_pos {
        Some(pos) if pos > 0 => Name::from_components(comps[..pos].iter().cloned()),
        _ => name.clone(),
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn strip_key_suffix_basic() {
        let name: Name = "/com/acme/alice/KEY/v=123/self".parse().unwrap();
        let stripped = strip_key_suffix(&name);
        let expected: Name = "/com/acme/alice".parse().unwrap();
        assert_eq!(stripped, expected);
    }

    #[test]
    fn strip_key_suffix_no_key() {
        let name: Name = "/com/acme/alice".parse().unwrap();
        let stripped = strip_key_suffix(&name);
        assert_eq!(stripped, name);
    }

    #[test]
    fn cert_to_did_doc_has_required_relationships() {
        use bytes::Bytes;
        let name: Name = "/com/acme/alice/KEY/v=1/self".parse().unwrap();
        let cert = Certificate {
            name: Arc::new(name),
            public_key: Bytes::from(vec![0u8; 32]),
            valid_from: 0,
            valid_until: u64::MAX,
            issuer: None,
            signed_region: None,
            sig_value: None,
            sig_type: ndn_packet::SignatureType::SignatureEd25519,
        };
        let doc = cert_to_did_document(&cert, None);
        let identity_name: Name = "/com/acme/alice".parse().unwrap();
        let expected_did = crate::did::encoding::name_to_did(&identity_name);
        assert_eq!(doc.id, expected_did);
        assert!(!doc.authentication.is_empty());
        assert!(!doc.assertion_method.is_empty());
        assert!(!doc.capability_invocation.is_empty());
        assert!(!doc.capability_delegation.is_empty());
        assert!(doc.key_agreement.is_empty()); // no X25519 supplied
    }

    #[test]
    fn trusted_approvers_round_trip() {
        use bytes::Bytes;

        // Minimal signed region: Name + SignatureInfo carrying the
        // trusted-approvers AdditionalDescription.
        let ad = encode_trusted_approvers_description(&["/lab/alice/devices/phone".to_string()]);
        let mut w = TlvWriter::new();
        w.write_nested(tlv_type::NAME, |w| {
            w.write_tlv(GENERIC_NAME_COMPONENT, b"lab");
            w.write_tlv(GENERIC_NAME_COMPONENT, b"alice");
            w.write_tlv(GENERIC_NAME_COMPONENT, KEY_COMPONENT);
            w.write_tlv(GENERIC_NAME_COMPONENT, b"k1");
        });
        w.write_nested(tlv_type::SIGNATURE_INFO, |w| {
            w.write_tlv(tlv_type::SIGNATURE_TYPE, &[5u8]);
            w.write_tlv(tlv_type::ADDITIONAL_DESCRIPTION, &ad);
        });
        let signed_region = w.finish();

        let cert = Certificate {
            name: Arc::new("/lab/alice/KEY/k1".parse().unwrap()),
            public_key: Bytes::from(vec![0u8; 32]),
            valid_from: 0,
            valid_until: u64::MAX,
            issuer: None,
            signed_region: Some(signed_region),
            sig_value: Some(Bytes::from_static(&[0u8; 64])),
            sig_type: ndn_packet::SignatureType::SignatureEd25519,
        };

        assert_eq!(
            trusted_approvers_from_cert(&cert),
            vec!["/lab/alice/devices/phone".to_string()]
        );
        let doc = cert_to_did_document(&cert, None);
        assert_eq!(
            doc.trusted_approvers(),
            vec!["/lab/alice/devices/phone".to_string()]
        );
    }

    #[test]
    fn cert_to_did_doc_with_x25519() {
        use bytes::Bytes;
        let name: Name = "/com/acme/alice/KEY/v=1/self".parse().unwrap();
        let cert = Certificate {
            name: Arc::new(name),
            public_key: Bytes::from(vec![0u8; 32]),
            valid_from: 0,
            valid_until: u64::MAX,
            issuer: None,
            signed_region: None,
            sig_value: None,
            sig_type: ndn_packet::SignatureType::SignatureEd25519,
        };
        let x25519 = [0xABu8; 32];
        let doc = cert_to_did_document(&cert, Some(&x25519));
        assert!(!doc.key_agreement.is_empty());
        assert_eq!(doc.verification_methods.len(), 2);
        let ka_vm = &doc.verification_methods[1];
        let crv = ka_vm
            .public_key_jwk
            .as_ref()
            .and_then(|jwk| jwk.get("crv"))
            .and_then(|v| v.as_str());
        assert_eq!(crv, Some("X25519"));
    }
}
