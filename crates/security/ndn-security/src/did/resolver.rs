...
impl UniversalResolver {
    /// Create a resolver with [`KeyDidResolver`] and a stub
    /// [`NdnDidResolver`] registered.
    ///
    /// The stub `did:ndn` resolver returns
    /// [`DidResolutionError::InternalError`] for every request —
    /// safe for `did:key`-only callers, loud for callers that
    /// expected `did:ndn` to work. Wire a real one via
    /// [`with_cert_fetcher`](Self::with_cert_fetcher).
    pub fn new() -> Self {
        let mut r = Self {
    ...
    /// **Recommended construction path** for callers that want
    /// `did:ndn:*` resolution to actually work.
    ///
    /// The `cert_fetcher` should be shared with the
    /// [`Validator`](crate::validator::Validator) so chain walks
    /// and DID resolutions de-duplicate against each other and
    /// warm the same [`CertCache`](crate::cert_cache::CertCache).
    pub fn with_cert_fetcher(cert_fetcher: Arc<crate::cert_fetcher::CertFetcher>) -> Self {
        let mut r = Self {
            resolvers: HashMap::new(),
        };
        r.register(KeyDidResolver);
        r.register(NdnDidResolver::default().with_cert_fetcher(cert_fetcher));
        r
    }
