...
impl MemKeyStore {
    ...
    /// Like [`add`](Self::add) but takes a pre-erased `Arc<dyn Signer>`;
    /// useful for PIB-loaded signers (OID-dispatched at read time).
    pub fn add_arc(&self, key_name: Arc<Name>, signer: Arc<dyn Signer>) {
        self.keys.insert(key_name, signer);
    }

    /// Any one registered signer — the daemon's identity when only one
    /// is installed.
    pub fn any_signer(&self) -> Option<Arc<dyn Signer>> {
        self.keys.iter().next().map(|r| Arc::clone(r.value()))
    }
}
