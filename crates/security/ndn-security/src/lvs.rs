...
//!
//! Imports the TLV binary format defined by python-ndn
//! (<https://python-ndn.readthedocs.io/en/latest/src/lvs/binary-format.html>)
//! and interoperable with NDNts `@ndn/lvs` and ndnd.
//!
//! Supports the full TLV parse, tree-walk evaluation of
//! `(data_name, key_name)` pairs, `ConstraintOption::Value` and
//! `ConstraintOption::Tag`, and `SignConstraint`. Named and temporary
...
pub enum LvsError {
    ...
    /// functions.
    #[error(
        "LVS schema uses user functions ($eq, $regex, …) not supported in this version; use LvsModel::decode() to inspect without enforcing"
    )]
    UserFunctionsNotSupported,
}
