//! SQLite-backed PIB, wire-compatible with `ndn-cxx`'s `pib-sqlite3`.
//!
//! Schema is replicated bit-for-bit — same tables, indexes, triggers,
//! and `wireEncode()`-based name storage — so an `ndnsec`-created
//! `pib.db` opens unchanged and vice versa. Any divergence (adding
//! `PRAGMA user_version`, storing names as URI strings, etc.) silently
//! breaks interop.
//!
//! Storage conventions (must match ndn-cxx):
//! - Default DB path: `$HOME/.ndn/pib.db` (see
//!   [`SqlitePib::open_default`] for `TEST_HOME` / CWD fallbacks).
//! - `Name` columns hold the TLV wire encoding (outer `0x07` + length +
//!   components), not URI strings.
//! - `key_bits` holds raw public-key bytes (DER SubjectPublicKeyInfo
//!   for ndn-cxx-issued keys).
//! - `certificate_data` holds the full Data wire encoding.
//! - `tpm_locator` is UTF-8 in a `BLOB` column.
//! - `PRAGMA foreign_keys=ON` per connection — required for `ON DELETE
//!   CASCADE` to work; without it `ndnsec` trips over orphan rows.
//! - Default-row invariants live in SQL triggers; `add_*` is a plain
//!   `INSERT`.
...
/// Schema replicated character-for-character from ndn-cxx 0.9.0.
/// Do not edit; divergence breaks `ndnsec` interop.
const DB_INIT: &str = r#"
