...
//!
//! Both the v2 multi-peer codec and the v3 self-only codec ([`crate::codec`])
//! — and the data-plane layers built on them in ndn-sync — encode NDN
//! NonNegativeIntegers and walk raw TLV cursors. This is the single home so a
//! third layer never adds a fourth copy.
//!
//! Heavier structured parsing should prefer [`ndn_tlv::TlvReader`] /
//! [`ndn_tlv::TlvWriter`]; these byte-cursor helpers exist for the tolerant,
//! allocation-light walks the v2 path already relied on.
//!
//! Pure `alloc` + `bytes`: no std, no async.
...
pub fn encode_nni(n: u64) -> Vec<u8> {
    if n <= 0xFF {
        vec![n as u8]
    } else if n <= 0xFFFF {
...
pub fn decode_nni(bytes: &[u8]) -> u64 {
    match bytes.len() {
        0 => 0,
        1 => bytes[0] as u64,
...
pub fn write_varnumber(buf: &mut BytesMut, n: u64) {
    if n < 0xFD {
        buf.put_u8(n as u8);
    } else if n <= 0xFFFF {
        buf.put_u8(0xFD);
...
pub fn read_tlv(cursor: &[u8]) -> Option<(u64, &[u8], &[u8])> {
...
/// reference decoders** the SVS wire is shared with — ndn-cxx `readVarNumber`
/// and ndnd `ReadTLNum` are both lenient the same way (all three encoders emit
/// minimal; none reject non-minimal on decode). Making this reader stricter than
/// the reference would only move the divergence, not close a gap that is
/// network-wide by nature.
///
...
pub fn read_varnumber(cursor: &[u8]) -> Option<(u64, &[u8])> {
