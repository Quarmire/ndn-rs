...
#              `LpLinkService` does, keyed on `FaceKind::scope()`.
#              The default LinkService selector picks Passthrough for
#              local kinds and Lp for non-local kinds, preserving the
#              F.04 "LP-encode-if-non-local-scope" rule bit-identically.
#
# Reverify recipe: RUST-UNIT. Runs the targeted ndn-transport
# link-service test module; no Docker, no toolchain beyond cargo.
#
# Exit codes:
#   0 — PASS (LinkService behaviours match the FaceKind::scope() classification)
...
#   - `default_link_service_matches_scope`
TESTS=(
    "passthrough_does_not_lp_encode"
    "lp_link_service_lp_encodes"
    "passthrough_writes_raw_lp_writes_wrapped"
...
if [ "$fail" -eq 0 ]; then
    echo "PASS: ARCH-4 — LinkService split is observable; Passthrough/Lp split matches FaceKind::scope()."
fi
exit "$fail"
