...
#   consumer crates (ndn-faces, ndn-mobile).  The SWIM protocol family
...
check_absent() {
    local pattern="$1"
    local desc="$2"
    local dirs="${3:-crates/ndn-discovery crates/ndn-faces crates/ndn-mobile}"
...
if [ -f "$REPO_ROOT/crates/ndn-faces/src/l2/ether_nd.rs" ]; then
    echo "FAIL: crates/ndn-faces/src/l2/ether_nd.rs still exists" | tee -a "$TRANSCRIPT"
    fail=1
else
