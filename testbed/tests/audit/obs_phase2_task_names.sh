...
check "face_write (engine.rs)" \
    "crates/engine/ndn-engine/src/engine.rs" \
    '"face_write"'
...
check "expiry pit" \
    "crates/engine/ndn-engine/src/expiry.rs" \
    '"pit"'
...
check "dvr_adv" \
    "crates/protocols/ndn-routing/src/protocols/dvr.rs" \
    '"dvr_adv"'

# nlsr_hello — Hello coordinator task
check "nlsr_hello" \
    "crates/protocols/ndn-routing/src/protocols/nlsr/hello.rs" \
    '"nlsr_hello"'

# nlsr_recompute — routing recompute task
check "nlsr_recompute" \
    "crates/protocols/ndn-routing/src/protocols/nlsr/protocol.rs" \
    '"nlsr_recompute"'
...
check "mgmt_request" \
    "binaries/ndn-fwd/src/mgmt_ndn.rs" \
    '"mgmt_request"'
