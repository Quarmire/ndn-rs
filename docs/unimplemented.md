...
- [x] **ndn-peek** — `crates/tooling/ndn-tools-core/src/peek.rs`
      Single and segmented fetch via `ForwarderClient`, ndn-cxx compatible naming.
- [x] **ndn-ping** — `crates/tooling/ndn-tools-core/src/ping.rs`
      Server and client modes; measures RTT, emits per-packet and summary events.
- [x] **ndn-put** — `crates/tooling/ndn-tools-core/src/put.rs`
