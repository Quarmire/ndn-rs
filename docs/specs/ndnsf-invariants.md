...
| NSF-A3 | `ACK`/`COORDINATION`/`RESPONSE` pass trust validation **and** NAC-ABE authorization before their payloads affect runtime state. | **Authorization:** `ndn-ndnsf::access` NAC-seals payloads under the service's KP-ABE attributes; only a satisfying `ServiceController`-issued key decrypts (unauthorized fails closed). **Trust validation:** `ndn-ndnsf::trust` is the message-validator mechanism — `sign_message` (publish as signed Data) + `verify_message` (validate the signature against trust anchors **and** require the signer be under the message's expected sender), fail closed. | ✅ authorization (`secure_four_phase_over_svs`); ✅ trust mechanism (`trust` unit tests: valid-accepted, wrong-sender/untrusted-signer/tampered rejected). ⚠️ *threading `sign_message`/`verify_message` through every driver leg is the remaining integration; the cleaner long-term home is `SvsPubSub`-level signing + the `IngestValidator` seam.* |
