...
- [ ] **9. Zero-component Name not rejected** — empty Name (no components) should be rejected for Interest/Data. (NDN Packet Format v0.3 §2)
...
- [ ] **11. ContentType values incomplete** — only BLOB(0) and KEY(2) handled; missing LINK(1), NACK(3), PREFIX_ANN(5), FLIC(1024). (NDN Packet Format v0.3 §6.3)
- [ ] **12. FinalBlockId not decoded** — Data MetaInfo FinalBlockId (TLV 0x1A) is not parsed; consumers cannot detect last segment. (NDN Packet Format v0.3 §6.2)
...
- [ ] **15. CanBePrefix / MustBeFresh semantics incomplete** — MustBeFresh is parsed but CS lookup may not filter on FreshnessPeriod expiry. CanBePrefix longest-match may be incomplete. (RFC 8569 §4.2)
...
- [ ] **18. InterestLifetime default** — spec default is 4000ms; need to verify current default. (NDN Packet Format v0.3 §5.2)
